...
# Open every bot database connection with durable SQLite settings.  The
# original code creates connections in many legacy helpers, so centralising
# this here protects all existing features without rewriting their schemas.
_sqlite_connect = sqlite3.connect


def _durable_sqlite_connect(*args, **kwargs):
    connection = _sqlite_connect(*args, **kwargs)
    database_name = args[0] if args else kwargs.get("database")
    configured_path = globals().get("DATABASE_PATH")
    if configured_path and database_name not in (":memory:", None):
        try:
            if os.path.abspath(os.fspath(database_name)) == os.path.abspath(configured_path):
                connection.execute("PRAGMA journal_mode=WAL")
                connection.execute("PRAGMA synchronous=FULL")
                connection.execute("PRAGMA foreign_keys=ON")
                connection.execute("PRAGMA busy_timeout=10000")
        except sqlite3.Error:
            # The caller still owns the connection and can report its normal
            # database error; never hide it behind a configuration failure.
            pass
    return connection


sqlite3.connect = _durable_sqlite_connect
...
# --- Configuration ---
# Keep credentials out of the source tree. Set BOT_TOKEN in the host's
# environment/secrets before starting the bot.
TOKEN = os.environ.get("BOT_TOKEN")
if not TOKEN:
    raise RuntimeError("BOT_TOKEN is not configured. Add it to the host's Secrets.")

def _read_admin_id():
    raw_admin_id = os.environ.get("ADMIN_ID", "").strip()
    if not raw_admin_id:
        raise RuntimeError("ADMIN_ID is not configured. Add it to the host's Secrets.")
    try:
...
# Common executable and binary file signatures.  These are intentionally
# kept separate from archive signatures because .zip uploads are supported.
MALWARE_SIGNATURES = [
    b'MZ',              # DOS/Windows PE executable
    b'\x7fELF',         # Linux/Unix ELF executable
    b'\xfe\xed\xfa\xce',# 32-bit Mach-O executable
    b'\xce\xfa\xed\xfe',# 32-bit Mach-O executable (alternate byte order)
...
# Archive signatures are kept separate from executable signatures.  Archives
# are not malware by themselves, but they must be inspected before extraction.
...
# Telegram's normal Bot API download limit is about 20 MB.  This hard cap is
# applied to every user, while FILE_ACTION_SIZE_LIMIT_BYTES remains the
# smaller quota for regular users.
TELEGRAM_DOWNLOAD_MAX_BYTES = int(
    os.environ.get("TELEGRAM_DOWNLOAD_MAX_BYTES", 20 * 1024 * 1024)
)
DOWNLOAD_CHUNK_SIZE = 64 * 1024
MAX_ZIP_ENTRIES = 1000
MAX_ARCHIVE_UNPACKED_BYTES = 64 * 1024 * 1024
MAX_ARCHIVE_COMPRESSION_RATIO = 1000
MAX_SCAN_TEXT_BYTES = 2 * 1024 * 1024
MAX_IMPORT_PARSE_BYTES = 4 * 1024 * 1024
...
class SafeThread(threading.Thread):
    """Prevent an exception in a worker from taking down the bot process."""

    def __init__(self, *args, **kwargs):
        # Legacy call sites omit daemon=True.  Make those workers non-blocking
        # during shutdown while preserving an explicit daemon choice.
    ...
    def run(self):
        try:
            ...
            logger.exception("Unhandled exception in background thread %s", self.name)


# All existing Thread(...) call sites, including old features, now get the
# same exception boundary without changing their public behaviour.
...
def init_db():
    ...
    try:
        ...
        # Manual uploads span two Telegram messages.  Persist that state so a
        # process restart does not orphan the first uploaded bot file.
        c.execute('''CREATE TABLE IF NOT EXISTS pending_uploads
                     (user_id INTEGER PRIMARY KEY,
                      mode TEXT NOT NULL,
                      stage TEXT NOT NULL,
                      pending_dir TEXT NOT NULL,
        ...
        c.execute("INSERT OR IGNORE INTO settings (key, value) VALUES ('bot_locked', '0')")
...
def load_data():
    ...
    global plan_id_counter, OWNER_ID
    logger.info("Loading data from database...")
    try:
        conn = sqlite3.connect(DATABASE_PATH, check_same_thread=False)
        c = conn.cursor()

        # Ownership changes are persisted in settings, so a restart does not
        # silently restore the old hard-coded owner.
        ...
        if owner_row:
            try:
                ...
                logger.warning("Ignoring invalid persisted owner_id: %r", owner_row[0])
        ...
        if lock_row is not None:
            try:
                global bot_locked
                bot_locked = str(lock_row[0]).strip().lower() in {"1", "true", "yes", "on"}
            except Exception:
                logger.warning("Ignoring invalid persisted bot_locked: %r", lock_row[0])
...
def clear_pending_upload_state(user_id):
    ...
    clear_persisted = globals().get("_clear_persisted_pending_upload")
    if clear_persisted:
        clear_persisted(user_id)
    if not state:
        return
...
def get_user_folder(user_id):
    ...
    root = os.path.realpath(UPLOAD_BOTS_DIR)
    user_folder = os.path.abspath(os.path.join(root, str(int(user_id))))
    if os.path.commonpath((root, user_folder)) != root:
        raise ValueError("Invalid user storage path")
    if os.path.islink(user_folder):
        raise OSError("User storage directory cannot be a symlink")
    os.makedirs(user_folder, mode=0o700, exist_ok=True)
    return user_folder


def sanitize_uploaded_filename(file_name):
    """Return a safe leaf filename and prevent traversal/control characters."""
    raw = str(file_name or "").replace("\\", "/")
    safe_name = posixpath.basename(raw).strip()
...
# Package lifecycle scripts are arbitrary code.  They are not needed for the
# bot's dependency discovery and must not run with the host environment.
NPM_FAST_INSTALL_FLAGS = [
    '--no-audit', '--no-fund', '--prefer-offline', '--ignore-scripts'
]

# --- Script Running Functions ---
def _user_process_env(user_folder):
    """Do not leak bot credentials into uploaded Python/JS processes."""
    allowed = {
    ...
    environment["HOME"] = user_folder
    environment["TMPDIR"] = user_folder
    environment["TEMP"] = user_folder
    environment["TMP"] = user_folder
    return environment
...
def _python_import_names(script_path):
    ...
    try:
        with open(script_path, 'r', encoding='utf-8', errors='ignore') as source:
            source_text = source.read(MAX_IMPORT_PARSE_BYTES + 1)
            if len(source_text) > MAX_IMPORT_PARSE_BYTES:
                logger.warning("Skipping import scan for oversized Python file %s", script_path)
                return []
            tree = ast.parse(source_text, filename=script_path)
...
def install_python_requirements_file(req_path, message):
    ...
    try:
        ...
        result = subprocess.run(
            ...
            env=_user_process_env(os.path.dirname(req_path))
        )
...
def install_missing_python_modules(script_path, message):
    ...
    try:
        ...
        result = subprocess.run(
            command, capture_output=True, text=True, check=False,
            encoding='utf-8', errors='ignore',
            env=_user_process_env(os.path.dirname(script_path))
        )
        if result.returncode == 0:
            bot.reply_to(
                message,
...
def _node_import_names(script_path):
    ...
    try:
        with open(script_path, 'r', encoding='utf-8', errors='ignore') as source:
            code = source.read(MAX_IMPORT_PARSE_BYTES + 1)
            if len(code) > MAX_IMPORT_PARSE_BYTES:
                logger.warning("Skipping import scan for oversized JS file %s", script_path)
                return []
    except OSError as e:
...
def install_missing_node_modules(script_path, user_folder, message):
    ...
    try:
        result = subprocess.run(
            ...
            encoding='utf-8', errors='ignore',
            env=_user_process_env(user_folder)
        )
        if result.returncode == 0:
            bot.reply_to(
                message,
...
def attempt_install_pip(module_name, message):
    ...
    try:
        ...
        result = subprocess.run(
            command, capture_output=True, text=True, check=False,
            encoding='utf-8', errors='ignore',
            env=_user_process_env(os.path.dirname(__file__))
        )
        if result.returncode == 0:
...
def _persist_owner_id(owner_id):
    """Persist ownership changes atomically for restart-safe administration."""
    with DB_LOCK:
        conn = sqlite3.connect(DATABASE_PATH, check_same_thread=False)
        try:
            conn.execute(
...
def _persist_bot_lock(locked):
    """Persist the lock toggle so a restart keeps the same access policy."""
    with DB_LOCK:
        conn = sqlite3.connect(DATABASE_PATH, check_same_thread=False)
        try:
            conn.execute(
                "INSERT INTO settings (key, value) VALUES ('bot_locked', ?) "
                "ON CONFLICT(key) DO UPDATE SET value=excluded.value",
                ("1" if locked else "0",)
            )
            conn.commit()
...
def _persist_pending_upload(user_id, state):
    """Persist the manual-upload state without storing file bytes in SQLite."""
    pending_file = state.get("pending_file") or {}
    pending_dir = os.path.abspath(str(state.get("pending_dir") or ""))
    pending_path = os.path.abspath(str(pending_file.get("path") or ""))
    if not pending_dir or not pending_path or not os.path.isfile(pending_path):
        raise ValueError("pending upload path is missing")
        ...
        try:
            conn.execute(
                """INSERT INTO pending_uploads
                   (user_id, mode, stage, pending_dir, pending_path,
                    file_name, file_ext, file_id, created_at)
                   VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)
                   ON CONFLICT(user_id) DO UPDATE SET
                     mode=excluded.mode,
                     stage=excluded.stage,
                     pending_dir=excluded.pending_dir,
                ...
                (
                    int(user_id),
                    str(state.get("mode") or "manual"),
                    str(state.get("stage") or "waiting_requirements"),
                    pending_dir,
                    pending_path,
                    os.path.basename(str(pending_file.get("file_name") or "")),
                    str(pending_file.get("file_ext") or ""),
                    str(pending_file.get("file_id") or ""),
...
def _clear_persisted_pending_upload(user_id):
    """Delete only the pending-upload row; never touch completed user data."""
    try:
        with DB_LOCK:
            conn = sqlite3.connect(DATABASE_PATH, check_same_thread=False)
            try:
                conn.execute("DELETE FROM pending_uploads WHERE user_id = ?", (int(user_id),))
                conn.commit()
            finally:
                conn.close()
    except Exception:
        logger.exception("Could not clear pending upload state for %s", user_id)


def _restore_pending_uploads():
    """Restore manual uploads whose files still exist after a restart."""
    try:
        conn = sqlite3.connect(DATABASE_PATH, check_same_thread=False)
        try:
            rows = conn.execute(
                """SELECT user_id, mode, stage, pending_dir, pending_path,
                          file_name, file_ext, file_id
                   FROM pending_uploads"""
            ).fetchall()
        finally:
            conn.close()

        upload_root = os.path.realpath(UPLOAD_BOTS_DIR)
        for (
            user_id, mode, stage, pending_dir, pending_path,
            file_name, file_ext, file_id
        ) in rows:
            real_path = os.path.realpath(pending_path)
            if (
                not os.path.isfile(real_path)
                or os.path.commonpath((upload_root, real_path)) != upload_root
            ):
                logger.warning(
                    "Removing stale pending upload row for user %s", user_id
                )
                _clear_persisted_pending_upload(user_id)
                continue
            pending_upload_modes[int(user_id)] = {
                "mode": mode,
                "stage": stage,
                "pending_dir": os.path.abspath(pending_dir),
                "pending_file": {
                    "path": os.path.abspath(pending_path),
                    "file_name": os.path.basename(file_name),
                    "file_ext": file_ext,
                    "file_id": file_id,
                },
            }
            logger.info("Restored pending manual upload for user %s", user_id)
    except Exception:
        logger.exception("Could not restore pending uploads")


_restore_pending_uploads()
...
def handle_zip_file(
    ...
    try:
        ...
        if isinstance(downloaded_file_content, (str, os.PathLike)):
            zip_path = os.path.abspath(os.fspath(downloaded_file_content))
        else:
            # Legacy callers may still pass bytes.  Keep compatibility, but
            # all normal Telegram uploads now use the streaming path.
            zip_path = os.path.join(
                temp_dir, os.path.basename(str(file_name_zip)) or "upload.zip"
            )
            with open(zip_path, 'wb', buffering=DOWNLOAD_CHUNK_SIZE) as new_file:
                for offset in range(0, len(downloaded_file_content), DOWNLOAD_CHUNK_SIZE):
                    new_file.write(downloaded_file_content[offset:offset + DOWNLOAD_CHUNK_SIZE])

        is_safe, reason = scan_file_for_malware(zip_path, file_name_zip, user_id)
        if not is_safe:
            bot.reply_to(
                message,
                render_body_text(f"🚨 Security Alert: {reason}"),
                parse_mode='HTML'
            )
            return
        
        with zipfile.ZipFile(zip_path, 'r') as zip_ref:
            valid_archive, archive_reason = _validate_zip_archive(zip_path)
            if not valid_archive:
                bot.reply_to(
                    message,
                    render_body_text(f"🚨 Security Alert: {archive_reason}"),
                    parse_mode='HTML'
                )
                return
        ...
        if dependency_mode == 'auto' and pkg_json:
            ...
            try:
                ...
                result = subprocess.run(
                    ...
                    env=_user_process_env(temp_dir)
                )
        ...
        for item_name in os.listdir(temp_dir):
            if item_name == os.path.basename(str(file_name_zip)):
                continue
            src_path = os.path.join(temp_dir, item_name)
            # Never remove an existing user file or directory while importing
            # a ZIP.  Preserve both projects by adding a collision suffix.
            safe_item_name = sanitize_uploaded_filename(item_name)
            dest_name = safe_item_name
            stem, extension = os.path.splitext(safe_item_name)
            suffix = 2
            dest_path = os.path.join(user_folder, dest_name)
            while os.path.lexists(dest_path):
                dest_name = f"{stem}_{suffix}{extension}"
                dest_path = os.path.join(user_folder, dest_name)
                suffix += 1
...
def _source_prefix(source, size=4096):
    """Read only a small header; never materialise an uploaded file."""
    if isinstance(source, (bytes, bytearray, memoryview)):
        return bytes(source[:size])
    with open(os.fspath(source), "rb", buffering=DOWNLOAD_CHUNK_SIZE) as handle:
        return handle.read(size)


def _source_chunks(source, limit=None):
    """Yield bounded chunks from bytes or a path for streaming scans."""
    if isinstance(source, (bytes, bytearray, memoryview)):
        payload = bytes(source)
        if limit is not None:
            payload = payload[:limit]
        for offset in range(0, len(payload), DOWNLOAD_CHUNK_SIZE):
            yield payload[offset:offset + DOWNLOAD_CHUNK_SIZE]
        return
...
def _zip_member_is_symlink(member):
    """Reject ZIP symlinks, which can escape the extraction directory."""
    mode = (member.external_attr >> 16) & 0o177777
    return stat.S_ISLNK(mode)


def _safe_archive_member_name(name):
    """Validate an archive name independently of the current user."""
    if not name or "\x00" in name:
        return False
    normalized = name.replace("\\", "/")
    if normalized.startswith("/") or normalized.startswith("../"):
        return False
    parts = [part for part in posixpath.normpath(normalized).split("/") if part]
    return bool(parts) and ".." not in parts


def _validate_zip_archive(zip_path):
    """Validate ZIP metadata before any extraction takes place."""
    try:
        with zipfile.ZipFile(zip_path, "r") as archive:
            members = archive.infolist()
            if len(members) > MAX_ZIP_ENTRIES:
                return False, "ZIP contains too many entries"

            total_unpacked = 0
            for member in members:
                if not _safe_archive_member_name(member.filename):
                    return False, f"Unsafe ZIP path: {member.filename!r}"
                if _zip_member_is_symlink(member):
                    return False, f"ZIP symlink is not allowed: {member.filename}"
                if member.flag_bits & 0x1:
                    return False, f"Encrypted ZIP entry is not allowed: {member.filename}"
                if member.file_size > MAX_ARCHIVE_UNPACKED_BYTES:
                    return False, f"ZIP entry is too large: {member.filename}"

                total_unpacked += member.file_size
                if total_unpacked > MAX_ARCHIVE_UNPACKED_BYTES:
                    return False, "ZIP expands beyond the configured safety limit"

                if member.file_size and member.compress_size:
                    ratio = member.file_size / member.compress_size
                    if ratio > MAX_ARCHIVE_COMPRESSION_RATIO:
                        return False, f"Suspicious compression ratio: {member.filename}"

                lower_name = member.filename.lower()
                if any(lower_name.endswith(ext) for ext in (
                    ".exe", ".dll", ".sys", ".scr", ".msi", ".msp",
                    ".com", ".cpl", ".jar", ".class", ".dex", ".apk",
                    ".dmg", ".iso", ".img", ".so", ".dylib",
                )):
                    return False, f"Executable inside ZIP: {member.filename}"

                # Read a bounded prefix to catch an executable with a
                # harmless-looking extension.  This is streaming and does
                # not extract the entry to disk.
                if not member.is_dir():
                    with archive.open(member, "r") as entry:
                        entry_header = entry.read(16)
                    if any(entry_header.startswith(signature)
                           for signature in MALWARE_SIGNATURES):
                        return False, f"Executable content inside ZIP: {member.filename}"
        return True, "Archive passed structural checks"
    except (zipfile.BadZipFile, OSError, RuntimeError) as exc:
        return False, f"Invalid or unreadable ZIP: {exc}"


def get_file_type(file_content):
    """Determine type from magic bytes, not only from a user filename."""
    header = _source_prefix(file_content, 16)
    signatures = (
        ...
        (ZIP_SIGNATURES[0], 'application/zip'),
        (b'Rar!', 'application/x-rar'),
        (SEVEN_ZIP_SIGNATURE, 'application/x-7z-compressed'),
    )
...
def _scan_stream_for_tokens(source, tokens, max_bytes=MAX_SCAN_TEXT_BYTES):
    """Find tokens across chunk boundaries with a bounded rolling window."""
    normalized_tokens = tuple(
        token.lower() if isinstance(token, bytes) else str(token).encode().lower()
        for token in tokens
    )
    overlap = max((len(token) for token in normalized_tokens), default=1) - 1
    tail = b""
    scanned = 0
    for chunk in _source_chunks(source, max_bytes):
...
def is_suspicious_file(file_content, file_name):
    """Perform bounded header, archive, and content checks."""
    file_lower = os.path.basename(str(file_name or "")).lower()
    suspicious_extensions = {
        '.exe', '.dll', '.bat', '.cmd', '.scr', '.com', '.pif',
        '.application', '.gadget', '.msi', '.msp', '.hta', '.cpl',
        '.msc', '.jar', '.class', '.bin', '.deb', '.rpm', '.apk',
        '.app', '.dmg', '.iso', '.img', '.sys', '.drv', '.ocx',
        '.so', '.dylib', '.dex', '.wasm',
    }
    if any(file_lower.endswith(ext) for ext in suspicious_extensions):
        return True, f"Hazardous executable extension: {file_name}"

    header = _source_prefix(file_content, 32)
    if any(header.startswith(signature) for signature in MALWARE_SIGNATURES):
        return True, f"Executable signature detected in {file_name}"

    if any(header.startswith(signature) for signature in RAR_SIGNATURES):
        return True, "RAR archives are rejected because safe inspection is unavailable"
    if header.startswith(SEVEN_ZIP_SIGNATURE):
        return True, "7z archives are rejected because safe inspection is unavailable"
    for signature, archive_name in COMPRESSED_SIGNATURES:
        if header.startswith(signature):
            return True, f"{archive_name} files are not accepted"

    if any(header.startswith(signature) for signature in ZIP_SIGNATURES):
        safe, reason = _validate_zip_archive(file_content)
        if not safe:
            return True, reason

    indicator = _scan_stream_for_tokens(file_content, ENCRYPTED_FILE_INDICATORS)
    if indicator:
        return True, f"Encrypted file indicator: {indicator.decode(errors='ignore')}"

    keyword = _scan_stream_for_tokens(file_content, SUSPICIOUS_KEYWORDS)
    if keyword:
        return True, f"Suspicious keyword found: {keyword.decode(errors='ignore')}"
    ...
    try:
        ...
        logger.warning("Could not determine file type: %s", exc)

    return False, "File passed security checks"


def scan_file_for_malware(file_content, file_name, user_id):
    """Scan a path or legacy bytes input; no account bypass is permitted."""
    try:
        is_suspicious, reason = is_suspicious_file(file_content, file_name)
    except (OSError, ValueError, TypeError) as exc:
        logger.warning("Scan failed for %s: %s", file_name, exc)
        return False, "File could not be safely inspected"

    if is_suspicious:
        logger.warning(
            "Security scan rejected %s from user %s: %s",
            file_name, user_id, reason
        )
        return False, f"Security violation: {reason}"
    return True, "File passed security checks"
...
def is_hosting_bot_file(file_content, file_name):
    """Detect hosting-panel signatures without reading an upload into RAM."""
    file_lower = os.path.basename(str(file_name or "")).lower()
    found_markers = set()

    if file_lower.endswith('.zip') or (
        isinstance(file_content, (str, os.PathLike))
        and str(file_content).lower().endswith('.zip')
    ):
        try:
            archive_source = (
                file_content
                if isinstance(file_content, (str, os.PathLike))
                else io.BytesIO(bytes(file_content))
            )
            with zipfile.ZipFile(archive_source, 'r') as archive:
                ...
                for member in archive.infolist():
                    if inspected >= 12:
                        break
                    if member.is_dir() or not member.filename.lower().endswith(('.py', '.js')):
                        continue
                    inspected += 1
                    try:
                        with archive.open(member, 'r') as entry:
                            tail = b""
                            for chunk in iter(lambda: entry.read(DOWNLOAD_CHUNK_SIZE), b""):
                                data = tail + chunk.lower()
                                for marker in HOSTING_BOT_MARKERS:
                                    if marker in data:
                                        found_markers.add(marker)
                                tail = data[-(max(map(len, HOSTING_BOT_MARKERS)) - 1):]
        ...
        for marker in HOSTING_BOT_MARKERS:
            if _scan_stream_for_tokens(
                file_content, (marker,), max_bytes=2 * 1024 * 1024
            ):
                found_markers.add(marker)
    ...
    if marker_count >= 2:
        return True
    if _scan_stream_for_tokens(
        file_content, (b'i.am mukesh file host', b"i'am mukesh file host"),
...
def stream_telegram_file_to_temp(file_id, target_dir, max_bytes):
    """Download a Telegram document to a bounded temporary file.

    ``TeleBot.download_file`` returns one large bytes object.  That is the
    source of the RAM spikes in the old handler, so use the file endpoint with
    ``stream=True`` and enforce the limit while writing.
    """
    os.makedirs(target_dir, exist_ok=True)
    fd, temp_path = tempfile.mkstemp(
        prefix=".telegram-upload-", suffix=".part", dir=target_dir
    )
    os.close(fd)
    try:
        ...
        if not remote_path:
            raise ValueError("Telegram returned an empty file path")

        url = f"https://api.telegram.org/file/bot{bot.token}/{remote_path}"
        with requests.get(
            url,
            stream=True,
            timeout=(15, 120),
            ...
            total = 0
            with open(temp_path, "wb", buffering=DOWNLOAD_CHUNK_SIZE) as output:
        ...
        try:
            ...
            logger.warning("Could not remove failed download %s", temp_path)
        raise
...
def forward_file_to_group(user_id, file_name, file_type, message, file_id=None):
    ...
    try:
        ...
        try:
            if file_id:
                # Telegram accepts an existing file_id directly.  Do not
                # download the same document a second time just to forward it
...
def handle_file_upload_doc(message):
    ...
    preserve_pending_file = False
    pending_state_to_clear = False
    ...
    if not file_name:
        ...
        return
    try:
        file_name = sanitize_uploaded_filename(file_name)
    except ValueError:
    ...
    try:
        ...
        user_folder = get_user_folder(user_id)
        download_limit = TELEGRAM_DOWNLOAD_MAX_BYTES
        if not is_file_action_quota_exempt(user_id):
            download_limit = min(download_limit, FILE_ACTION_SIZE_LIMIT_BYTES)
        downloaded_file_path = stream_telegram_file_to_temp(
            doc.file_id, user_folder, download_limit
        )

        if (
            not is_file_action_quota_exempt(user_id)
            and os.path.getsize(downloaded_file_path) > FILE_ACTION_SIZE_LIMIT_BYTES
        ):
            bot.edit_message_text(
                render_body_text(
                    "⚠️ File too large. Regular users can upload a maximum of 1 MB."
        ...
        is_safe, reason = scan_file_for_malware(
            downloaded_file_path, file_name, user_id
        )
        if not is_safe:
            bot.edit_message_text(
                render_body_text(
        ...
        if (
            ...
            and is_hosting_bot_file(downloaded_file_path, file_name)
        ):
            clear_pending_upload_state(user_id)
            ban_user(
                user_id,
        ...
        if upload_mode == "manual" and upload_stage == "waiting_file":
            # Keep pending data under the persistent upload directory rather
            # than /tmp, so a restart does not lose the first file.
            pending_dir = None
            try:
                pending_dir = tempfile.mkdtemp(
                    prefix=f".user_{user_id}_manual_", dir=user_folder
                )
                pending_path = os.path.join(
                    pending_dir, os.path.basename(file_name)
                )
                os.replace(downloaded_file_path, pending_path)
                downloaded_file_path = None
                upload_state.update({
                    "mode": "manual",
                ...
                _persist_pending_upload(user_id, upload_state)
            except Exception:
                if pending_dir and os.path.exists(pending_dir):
        ...
        if is_requirements_upload:
            ...
            os.replace(downloaded_file_path, req_path)
            downloaded_file_path = None
            ...
            try:
                downloaded_file_path = os.path.abspath(pending_path)
                preserve_pending_file = True
                pending_state_to_clear = True
        ...
        if (
            ...
            # Do not delete a durable manual-upload file just because quota
            # reservation failed.  The user can retry after an admin reset.
            if not pending_state_to_clear:
                clear_pending_upload_state(user_id)
            bot.reply_to(message, file_action_limit_message(), parse_mode='HTML')
            return
        ...
        if file_ext == '.zip':
            handle_zip_file(
                downloaded_file_path, file_name, message,
            ...
            file_path = os.path.join(user_folder, file_name)
            os.replace(downloaded_file_path, file_path)
            downloaded_file_path = None
        ...
        try:
            if "file is too big" in str(e).lower():
                bot.reply_to(message, "❌ Telegram file is too large to download.")
            else:
                bot.reply_to(message, "❌ Telegram download failed. Please try later.")
        except Exception:
            logger.exception("Could not send Telegram upload error to %s", user_id)
    except Exception as e:
        logger.error(f"❌ General error handling file for {user_id}: {e}", exc_info=True)
        try:
            bot.reply_to(message, "❌ Unexpected upload error. Please try again later.")
        except Exception:
            logger.exception("Could not send upload error to user %s", user_id)
    finally:
        # A pending manual file belongs to the durable workflow and must stay
        # on disk.  Every other temporary download is removed here.
        if downloaded_file_path and not preserve_pending_file:
            try:
                if os.path.isfile(downloaded_file_path):
                    os.remove(downloaded_file_path)
            except OSError:
                logger.exception(
                    "Could not remove temporary upload %s", downloaded_file_path
                )
##
# ==========================================
...
if __name__ == '__main__':
    ...
    # Fail fast on an invalid token instead of keeping a Render service alive
    # while Telegram repeatedly returns 401 Unauthorized.
    try:
        bot.get_me()
    except telebot.apihelper.ApiTelegramException as exc:
        if "401" in str(exc) or "Unauthorized" in str(exc):
            raise RuntimeError(
                "Telegram rejected BOT_TOKEN. Generate a fresh token in BotFather "
